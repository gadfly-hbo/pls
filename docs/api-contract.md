...
- **Deterministic Fallback**：默认 LLM 口径为 `minimax-m3`，但当前实现未接入外部 LLM；provider 不可用或未配置时返回 `deterministic_fallback`，并在 `qualityFlags` 中标记。
...
```json
{
  ...
  "marketContext": {
    ...
    "marketingEventId": "event_001",
    "businessScenarioId": "scenario_001",
    "contextText": "..."
  },
...
- `provider` / `modelVersion`：记录实际生成模型，当前固定为 `deterministic_fallback` / `deterministic-fallback-0.1`。
- `qualityFlags`：至少包含 `deterministic_fallback_used`（等价于 PRD 中的 `llm_unavailable_fallback_used`）。输入质量不足时可能附加 `strategy_text_too_short`、`missing_target_agent_profile`、`missing_market_context`。
