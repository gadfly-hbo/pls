...
- **目标用户 Agent**：一期默认支持三大人群模板（`质感流行派`、`都市体面家`、`百搭优选客`），同时支持手写 `manual_persona`。
- **LLM 优先 + Deterministic Fallback**：默认调用 Minimax LLM（`provider=minimax`，`modelVersion=minimax-m3`）。当 `MINIMAX_API_KEY` 未配置、provider 不可用、请求超时、返回非法 JSON 或模型层校验失败时，自动回退到 `deterministic_fallback`，并在 `qualityFlags` 中同时标记 `deterministic_fallback_used` 和 `llm_unavailable_fallback_used`。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/simulated-market/agent-templates` | 返回可用目标用户 Agent 模板，一期含三大人群 |
...
- `invalid_input` / 400：`sourceType` 非法、`strategyText` 缺失、`targetAgentSet` 为空、Agent 缺少 `agentId`/`name`、请求体不是合法 JSON。
- `not_found` / 404：`runId` 不存在或属于其他 workspace。
- `conflict` / 409：`Idempotency-Key` 已用于不同请求体。

### 13.4 环境变量与 Provider 配置

模拟市场 LLM provider 在 server 启动时读取环境变量（server 重启后生效）。`MINIMAX_API_KEY` 仅在 server 端读取，绝不进入日志、DB、audit meta 或前端响应。

| 环境变量 | 默认值 | 是否必填 | 说明 |
|---|---|---|---|
| `MINIMAX_API_KEY` | — | 真实 LLM 调用时必填 | Minimax API key。 |
| `MINIMAX_API_HOST` | `https://api.minimaxi.com` | 否 | Minimax API host，可覆盖为其他兼容 endpoint。 |
| `SIMULATED_MARKET_MODEL` | `minimax-m3` | 否 | 调用 Minimax 时使用的模型名。 |
...
- 默认 CI / `npm run smoke:simulated-market` 使用 `SIMULATED_MARKET_FAKE_LLM=true`，验证 LLM 成功路径，不依赖真实 key 或网络。
- 真实 Minimax 验证为可选：仅当 `RUN_SIMULATED_MARKET_LIVE_LLM=1` 且 `MINIMAX_API_KEY` 同时配置时，才会在 smoke 中调用真实网络；否则该 Phase 自动跳过并说明原因，不会导致 smoke 失败。
- 手动运行真实 LLM smoke 示例：`cd apps/server && RUN_SIMULATED_MARKET_LIVE_LLM=1 MINIMAX_API_KEY=<key> npm run smoke:simulated-market`。
