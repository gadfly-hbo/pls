...
| `source` | 是 | 数据来源或生成方式 |
| `sampleSize` | 条件必填 | 真实聚合数据必须提供，预测结果可为空 |
| `timeWindow` | 条件必填 | 真实聚合数据必须提供，预测结果可为空 |
...
DMP 字段进入 PLS 前必须先本地聚合，再映射到内部标签。
...
- 禁止把手机号、姓名、地址、订单号、账号 ID 或用户级明细作为标签值。
- 禁止用不可解释的 DMP 黑盒字段直接驱动模型，必须保留映射说明和置信度。
